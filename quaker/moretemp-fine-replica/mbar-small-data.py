...
for n in range(10):
    ...
    # variable 'n' decided which segment of replica exchange to take
    for temp in rep_temp_list:
        data = np.genfromtxt('theta{:3.1f}.txt'.format(temp))
        data = data.reshape((-1, 240))
        data_i = np.mean(data, axis=1)
        theta_ik.append( data_i[ len(lines) - T + 200*n : len(lines) - T + 200*(n+1) ] )
    ...
    for k, temp in enumerate(rep_temp_list):
        ...
        VO_ik.append( lines[ len(lines) - T + 200*n : len(lines) - T + 200*(n+1) ] )
        dtheta_i = np.array(theta_ik[k + 3*N_bias]) - th
        UO_ik.append( lines[ len(lines) - T + 200*n : len(lines) - T + 200*(n+1) ] - 0.5 * k_list[k + 3*N_bias] * np.square(dtheta_i) )
...
f_av=np.mean(f_tot, axis=0)
df_av=np.std(f_tot, axis=0)
plt.errorbar(theta_axis, f_av, df_av, ecolor='g')
plt.plot(theta_axis, f_av, 'ro')
plt.show()

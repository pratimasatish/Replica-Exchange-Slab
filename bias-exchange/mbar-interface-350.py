...
namelist_350start = np.array( [-0.750, -0.700, -0.690, -0.680, -0.670, -0.660, -0.650, -0.640, -0.630, -0.620, -0.610, -0.600, -0.575, -0.550] )
namelist_350mid = np.array( [-0.600, -0.580, -0.560, -0.540, -0.520, -0.500, -0.480, -0.460, -0.440, -0.420, -0.400, -0.380, -0.360, -0.340] )
namelist_350last = np.array( [-0.370, -0.350, -0.330, -0.310, -0.290, -0.270, -0.250, -0.230, -0.210, -0.190, -0.170, -0.150, -0.130, -0.110] )
T_bias = 5000
N_bias = len(namelist_350start)
N_int = len(namelist_350start)

full_namelist = np.concatenate(( namelist_350start, namelist_350mid, namelist_350last ))
temp_list = np.concatenate(( np.ones(N_int) * 350.0, np.ones(N_int) * 350.0, np.ones(N_int) * 350.0 ))
...
for biasval in namelist_350start:
    data = np.genfromtxt('interface-start350K/theta{:1.3f}.txt'.format(biasval))
    data = data.reshape((-1, 240))
    data_i = np.mean(data, axis=1)
    theta_ik.append( data_i )

for biasval in namelist_350mid:
    data = np.genfromtxt('interface-mid350K/theta{:1.3f}.txt'.format(biasval))
    data = data.reshape((-1, 240))
    data_i = np.mean(data, axis=1)
    theta_ik.append( data_i )

for biasval in namelist_350last:
    data = np.genfromtxt('interface-last350K/theta{:1.3f}.txt'.format(biasval))
    data = data.reshape((-1, 240))
    data_i = np.mean(data, axis=1)
    theta_ik.append( data_i )
...
N_bias = 0
for k, th in enumerate(namelist_350start):
    lines = np.genfromtxt("interface-start350K/pot-new.{:1.3f}".format(th))
...
for k, th in enumerate(namelist_350mid):
    lines = np.genfromtxt("interface-mid350K/pot-new.{:1.3f}".format(th))
    VO_ik.append( lines[5001:] )
    dtheta_i = np.array(theta_ik[k + N_bias + N_int]) - th
#     UO_ik.append( lines[5001:] - 0.5 * k_list[k + N_bias + N_int] * np.square(dtheta_i) )
    UO_ik.append( lines[5001:] )

for k, th in enumerate(namelist_350last):
    lines = np.genfromtxt("interface-last350K/pot-new.{:1.3f}".format(th))
    VO_ik.append( lines[5001:] )
...
disord_min = np.mean(theta_axis[np.where(np.abs(theta_axis - -0.290) < 0.02)])
ord_min = np.mean(theta_axis[np.where(np.abs(theta_axis - -0.750) < 0.01)])

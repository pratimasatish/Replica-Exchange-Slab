...
namelist_375start = np.array( [-0.750, -0.700, -0.690, -0.680, -0.670, -0.660, -0.650, -0.640, -0.630, -0.620, -0.610, -0.600, -0.575, -0.550] )
namelist_375mid = np.array( [-0.600, -0.580, -0.560, -0.540, -0.520, -0.500, -0.480, -0.460, -0.440, -0.420, -0.400, -0.380, -0.360, -0.340] )
namelist_375last = np.array( [-0.370, -0.350, -0.330, -0.310, -0.290, -0.270, -0.250, -0.230, -0.210, -0.190, -0.170, -0.150, -0.130, -0.110] )
T_bias = 5000
N_bias = len(namelist_375start)
N_int = len(namelist_375start)

full_namelist = np.concatenate(( namelist_375start, namelist_375mid, namelist_375last ))
temp_list = np.concatenate(( np.ones(N_int) * 375.0, np.ones(N_int) * 375.0, np.ones(N_int) * 375.0 ))
...
for biasval in namelist_375start:
    data = np.genfromtxt('interface-start375K/theta{:1.3f}.txt'.format(biasval))
    data = data.reshape((-1, 240))
    data_i = np.mean(data, axis=1)
    theta_ik.append( data_i )

for biasval in namelist_375mid:
    data = np.genfromtxt('interface-mid375K/theta{:1.3f}.txt'.format(biasval))
    data = data.reshape((-1, 240))
    data_i = np.mean(data, axis=1)
    theta_ik.append( data_i )

for biasval in namelist_375last:
    data = np.genfromtxt('interface-last375K/theta{:1.3f}.txt'.format(biasval))
    data = data.reshape((-1, 240))
    data_i = np.mean(data, axis=1)
    theta_ik.append( data_i )
...
N_bias = 0
for k, th in enumerate(namelist_375start):
    lines = np.genfromtxt("interface-start375K/pot-new.{:1.3f}".format(th))
...
for k, th in enumerate(namelist_375mid):
    lines = np.genfromtxt("interface-mid375K/pot-new.{:1.3f}".format(th))
    VO_ik.append( lines[5001:] )
    dtheta_i = np.array(theta_ik[k + N_bias + N_int]) - th
#     UO_ik.append( lines[5001:] - 0.5 * k_list[k + N_bias + N_int] * np.square(dtheta_i) )
    UO_ik.append( lines[5001:] )

for k, th in enumerate(namelist_375last):
    lines = np.genfromtxt("interface-last375K/pot-new.{:1.3f}".format(th))
    VO_ik.append( lines[5001:] )
...
disord_min = np.mean(theta_axis[np.where(np.abs(theta_axis - -0.237) < 0.01)])
ord_min = np.mean(theta_axis[np.where(np.abs(theta_axis - -0.720) < 0.01)])
...
# plt.plot(theta_axis, line+14.5, color='r', linestyle='--', linewidth=3)
# plt.xlim(-1.0, 0.0)
# # plt.ylim(-0.2, 20)
# plt.xlabel(r'$\theta_z$', fontsize=28)

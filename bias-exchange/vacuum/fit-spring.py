...
plt.plot(theta_axis, test_data, 'r', linewidth=4, alpha=0.5, label=r'$\textbf{{ fit for }} \boldsymbol{{\sigma}} \textbf{{ = {:1.3f} kcal/mol-nm}}$'.format( sigma_opt / trans_beta ))
plt.plot(theta_axis, f_i, 'bv', label=r'$\textbf{original } \boldsymbol{\beta F}$')
plt.plot(theta_axis, f_i, 'b', linewidth=3, alpha=0.5)
plt.xlabel(r'$\boldsymbol{\langle\theta_z\rangle}', fontsize=28)
plt.ylabel(r'$\boldsymbol{\beta F(\langle\theta_z\rangle)}$', fontsize=28)
plt.legend(loc='best', fontsize=20)
plt.show()

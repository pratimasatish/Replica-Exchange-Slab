...
parser.add_argument("-temp", type=int, default=350, help="temperature with full data range to use for extrapolation")
args = parser.parse_args()

# initialise list of temperatures 
kB = 1.3806503 * 6.0221415 / 4184.0

namelist_365start = np.array( [-0.750, -0.700, -0.690, -0.680, -0.670, -0.660, -0.650, -0.640, -0.630, -0.620, -0.610, -0.600, -0.575, -0.550] )
namelist_365mid = np.array( [-0.600, -0.580, -0.560, -0.540, -0.520, -0.500, -0.480, -0.460, -0.440, -0.420, -0.400, -0.380, -0.360, -0.340] )
namelist_365last = np.array( [-0.370, -0.350, -0.330, -0.310, -0.290, -0.270, -0.250, -0.230, -0.210, -0.190, -0.170, -0.150, -0.130, -0.110] )
T_bias = 5000
N_bias = len(namelist_365start)
N_int = len(namelist_365start)

full_namelist = np.concatenate(( namelist_365start, namelist_365mid, namelist_365last ))
temp_list = np.concatenate(( np.ones(N_int) * 365.0, np.ones(N_int) * 365.0, np.ones(N_int) * 365.0 ))
...
for biasval in namelist_365start:
    data = np.genfromtxt('interface-start365K/theta{:1.3f}.txt'.format(biasval))
    data = data.reshape((-1, 240))
    data_i = np.mean(data, axis=1)
    theta_ik.append( data_i )

for biasval in namelist_365mid:
    data = np.genfromtxt('interface-mid365K/theta{:1.3f}.txt'.format(biasval))
    data = data.reshape((-1, 240))
    data_i = np.mean(data, axis=1)
    theta_ik.append( data_i )

for biasval in namelist_365last:
    data = np.genfromtxt('interface-last365K/theta{:1.3f}.txt'.format(biasval))
    data = data.reshape((-1, 240))
    data_i = np.mean(data, axis=1)
    theta_ik.append( data_i )
...
N_bias = 0
for k, th in enumerate(namelist_365start):
    lines = np.genfromtxt("interface-start365K/pot-new.{:1.3f}".format(th))
...
#     UO_ik.append( lines[5001:] - 0.5 * k_list[k + N_bias] * np.square(dtheta_i) )
    UO_ik.append( lines[5001:] )

for k, th in enumerate(namelist_365mid):
    lines = np.genfromtxt("interface-mid365K/pot-new.{:1.3f}".format(th))
    VO_ik.append( lines[5001:] )
    dtheta_i = np.array(theta_ik[k + N_bias + N_int]) - th
#     UO_ik.append( lines[5001:] - 0.5 * k_list[k + N_bias + N_int] * np.square(dtheta_i) )
    UO_ik.append( lines[5001:] )

for k, th in enumerate(namelist_365last):
    lines = np.genfromtxt("interface-last365K/pot-new.{:1.3f}".format(th))
    VO_ik.append( lines[5001:] )
...
disord_min = np.mean(theta_axis[np.where(np.abs(theta_axis - -0.247) < 0.01)])
ord_min = np.mean(theta_axis[np.where(np.abs(theta_axis - -0.734) < 0.01)])
disord_F = np.mean(f_i[np.where(np.abs(theta_axis - disord_min) < 0.01)])
ord_F = np.mean(f_i[np.where(np.abs(theta_axis - ord_min) < 0.01)])
print "ord min = {} disord min = {}".format(ord_min, disord_min)
...
# plt.plot(theta_axis, line+16.5, color='r', linestyle='--', linewidth=3)
# plt.xlim(-1.0, 0.0)
# # plt.ylim(-0.2, 20)
# plt.xlabel(r'$\theta_z$', fontsize=28)
# plt.ylabel(r'$F(\theta_z) \textrm{ (in kcal/mol)}$', fontsize=28)
# # plt.legend(loc='best')
# plt.show()
# # plt.savefig('free-en-365K.png')
...
print 'ord indices', ord_indices, ord_indices.shape
print 'chem pot aka slope {}'.format(slope)
...
# plt.show()
# plt.savefig('prob-dist-365K.png')
...
# populate diagonal blocks in MBAR array
test_temp = 371
beta_transition = 1/(kB * test_temp)
for k in range(K):
...
plt.title('T={}K'.format(test_temp))
# plt.legend(loc='best')
plt.show()
# plt.savefig('predicted-coex-from-365K.png')
...
# plt.ylabel(r'$P(\theta_z)$', fontsize=28)
# plt.show()
# plt.savefig('coex-pdist-365K.png')

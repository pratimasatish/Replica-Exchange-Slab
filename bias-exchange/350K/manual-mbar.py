...
namelist = [-0.750, -0.650, -0.600, -0.575, -0.550, -0.525, -0.500, -0.450, -0.375, -0.275, -0.200, -0.100]
...
#     data = np.genfromtxt('theta.old{:1.3f}.txt'.format(biasval))
...
# for index in range(len(f_i)):
#     print theta_axis[index], f_i[index]
...
disord_min = np.mean(theta_axis[np.where(np.abs(theta_axis - -0.275) < 0.005)])
ord_min = np.mean(theta_axis[np.where(np.abs(theta_axis - -0.75) < 0.005)])
disord_F = np.mean(f_i[np.where(np.abs(theta_axis - -0.275) < 0.005)])
ord_F = np.mean(f_i[np.where(np.abs(theta_axis - -0.75) < 0.005)])

disord_E = ave_u[ np.where(np.equal(namelist, np.ones(K) * -0.275)) ]
ord_E = ave_u[ np.where(np.equal(namelist, np.ones(K) * -0.75)) ] 
...
# plot per site probability distributions for bias values with O-Do interface present
interface_list = [-0.600, -0.575, -0.550, -0.525, -0.500]
interface_list = np.array(interface_list)
K_int = len(interface_list)
color_list=iter(plt.cm.rainbow(np.linspace(0,1,K_int)))
for ind in range(K_int):
    c = next(color_list)
    index = [i for i, j in enumerate(namelist) if j == interface_list[ind]][0]
    plt.hist(th_ik[index, :], bins=bin_centers, normed=True, label=r'$\theta = {}$'.format(namelist[index]), histtype='stepfilled', alpha=0.5, color=c)
#     plt.hist(th_ik[index, :], bins=bin_centers, label=r'$\theta = {}$'.format(namelist[index]), histtype='stepfilled', alpha=0.5)
    plt.plot(bin_centers, (np.sqrt(beta * k_list[k])) * np.exp(-beta * new_uo[index, :]), linewidth=3, color=c)
plt.xlabel(r'$\theta$', fontsize=28)
